    ...
    return (
      <div className="min-h-screen bg-nox-black flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link to="/events" className="text-nox-primary hover:underline">
  ...
  return (
    <div className="min-h-screen bg-nox-black py-6 px-4">
      <div className="max-w-2xl mx-auto">
        <Link to="/events" className="text-nox-primary/80 hover:text-nox-primary text-sm mb-4 inline-block">
          ← Retour aux événements
        </Link>
        ...
        <div className="relative rounded-xl overflow-hidden mb-6">
          {event.image ? (
            ...
            <div className="w-full h-56 bg-nox-dark flex items-center justify-center">
              <span className="text-6xl">🎵</span>
            </div>
          )}
          <div className="absolute bottom-2 right-2 bg-nox-primary text-nox-black px-3 py-1 rounded-full font-bold">
            {event.price ?? 0}€
          </div>
        </div>

        <h1 className="text-2xl font-bold text-nox-white mb-2">{event.title}</h1>
        {event.genre && (
          <span className="inline-block bg-nox-gray px-2 py-1 rounded text-sm text-nox-primary mb-4">
            {event.genre}
          </span>
        )}

        <div className="space-y-2 text-nox-white/80 mb-6">
        ...
        {event.description && (
          <p className="text-nox-white/90 mb-6 whitespace-pre-wrap">{event.description}</p>
        )}
        ...
        {event.capacity != null && (
          <div className="mb-6">
            ...
            <div className="w-full bg-nox-gray rounded-full h-2 mt-1">
              <div
                className="bg-nox-primary h-2 rounded-full"
        ...
        {user?.isAuthenticated && (
          <button
            className="w-full bg-nox-primary text-nox-black font-bold py-3 rounded-lg hover:bg-orange-500 transition"
            disabled
          >
