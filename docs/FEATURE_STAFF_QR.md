...
- **Phase de test (app)** : si **`SCAN_TICKET_TEST_SECRET`** est défini sur le serveur (≥ 8 car.) et **`EXPO_PUBLIC_SCAN_TICKET_TEST_SECRET`** dans le build mobile avec la **même** valeur, l’écran **Scan billets** affiche un interrupteur *Test : scan hors jour événement* ; lorsqu’il est actif, les requêtes envoient le secret (sans ouvrir le scan à tout le monde comme `ALLOW_ANY_DAY` global). En **`__DEV__`**, l’interrupteur est visible même sans variable `ENABLE` ; option **`EXPO_PUBLIC_ENABLE_SCAN_TEST_TOGGLE=true`** pour les builds preview sans secrets forcés visibles en dev uniquement — le secret reste requis pour que le switch fonctionne.
