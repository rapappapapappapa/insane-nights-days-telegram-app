...
### Modifié
- **Feed (`FeedPage` / `HomePage` / `WelcomePage`)** : au retour sur l’écran, **rafraîchissement du feed** + **paramètre de cache-bust** sur l’avatar des **vos** posts — la photo de profil DJ / organisateur mise à jour s’affiche sans rester sur l’ancienne image (cache React Native + données API).
- **`BookerEventDashboardPage` (Android)** : **`KeyboardAvoidingView`** sans `behavior="height"` (évite de rogner le scroll) ; **`ScrollView`** avec **`flexGrow` / padding** bas pour l’étape DJs ; normalisation numérique de **`slotIndex`** (évite que le 2e DJ remplace le 1er) ; paramètre **`resumeStep`** (2 ou 3) renvoyé depuis **`VenueProfilePage`** / **`DjProfilePage`** pour forcer l’étape au retour de sélection.
